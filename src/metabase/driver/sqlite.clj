...
(defmethod driver/details-fields :sqlite [_]
  [{:name         "db"
...
(defmethod driver/current-db-time :sqlite [driver database]
  (driver.common/current-db-time sqlite-db-time-query sqlite-date-formatters driver database))

(defmethod sql-jdbc.sync/active-tables :sqlite [driver metadata]
