...
(defmethod driver/details-fields :h2 [_]
  [{:name         "db"
...
(def ^:private h2-date-formatters (driver.common/create-db-time-formatters date-format-str))
(def ^:private h2-db-time-query  (format "select formatdatetime(current_timestamp(),'%s') AS VARCHAR" date-format-str))

(defmethod driver/current-db-time :h2 [driver database]
  (driver.common/current-db-time h2-db-time-query h2-date-formatters driver database))
...
(defmethod sql-jdbc.sync/active-tables :h2 [driver metadata]
  (sql-jdbc.sync/post-filtered-active-tables driver metadata))
