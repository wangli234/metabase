...
(defmethod driver/details-fields :redshift [_]
...
(def ^:private redshift-date-formatters (driver.common/create-db-time-formatters "yyyy-MM-dd HH:mm:ss.SSS zzz"))
(def ^:private redshift-db-time-query "select to_char(current_timestamp, 'YYYY-MM-DD HH24:MI:SS.MS TZ')")

(defmethod driver/current-db-time :redshift [driver database]
  (driver.common/current-db-time redshift-db-time-query redshift-date-formatters driver database))
