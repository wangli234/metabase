...
(defmethod sql-jdbc.conn/connection-details->spec :sqlserver [_ {:keys [user password db host port instance domain ssl]
  ...
  (-> {:applicationName config/mb-app-id-string
       :classname       "com.microsoft.sqlserver.jdbc.:sqlserver"
...
(defmethod driver/current-db-time :sqlserver [driver db]
  (driver.common/current-db-time sqlserver-db-time-query sqlserver-date-formatters driver db))
...
(defmethod driver/details-fields :sqlserver [_]
