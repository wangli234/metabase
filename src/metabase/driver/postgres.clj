...
(def ^:private pg-date-formatters (driver.common/create-db-time-formatters "yyyy-MM-dd HH:mm:ss.SSS zzz"))
(def ^:private pg-db-time-query "select to_char(current_timestamp, 'YYYY-MM-DD HH24:MI:SS.MS TZ')")

(defmethod driver/current-db-time :postgres [driver database]
  (driver.common/current-db-time pg-db-time-query pg-date-formatters driver database))


(defmethod driver/details-fields :postgres [_]
...
(defmethod sql-jdbc.sync/column->special-type :postgres [driver database-type _]
