...
(sql.tx/add-standalone-comment-extensions! :postgres)
...
(defmethod tx/database->connection-details :postgres [_ context {:keys [database-name]}]
  (merge
...
(defmethod ddl/drop-db-ddl-statements :postgres [driver {:keys [database-name], :as dbdef} & options]
  ...
  (cons
   ...
   (apply ddl/drop-db-ddl-statements :sql-jdbc/test-extensions driver dbdef options)))
