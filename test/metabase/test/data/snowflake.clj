...
(defmethod tx/database->connection-details :snowflake [_ context {:keys [database-name]}]
  (merge
...
(defn- no-db-connection-spec
  ...
  []
  (sql-jdbc.conn/connection-details->spec :snowflake (tx/database->connection-details :snowflake nil)))
