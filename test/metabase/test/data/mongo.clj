...
(defmethod tx/database->connection-details :mongo [_ _ dbdef]
  {:dbname (tx/escaped-name dbdef)
   :host   "localhost"})

(defn- destroy-db! [driver dbdef]
  (with-open [mongo-connection (mg/connect (tx/database->connection-details driver dbdef))]
...
(defmethod tx/create-db! :mongo
  ...
  (with-mongo-connection [mongo-db (tx/database->connection-details driver dbdef)]
